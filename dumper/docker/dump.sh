...
for layer in $(pwd)/data/image/*/layer.tar; do
    ...
    if [ -n "$(tar -tf ${layer} | grep "opt/node-v${IMAGE_ARG_NODE_VERSION:-10.15.3}-linux-x64")" ]; then echo found node-v${IMAGE_ARG_NODE_VERSION:-10.15.3}-linux-x64 in ${layer}; LAYERS+=(${layer}); fi
